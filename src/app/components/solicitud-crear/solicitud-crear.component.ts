...
export class SolicitudCrearComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reactiveForm();
    this.setCapitalizacionDefault();
    this.setDefaultValueAndBlockCantPeriodo();
  ...
  reactiveForm() {
    this.myForm = this.fb.group({
      ...
      capitalizacion: [{ value: '', disabled: true }, [Validators.required]],
      plazo: ['', [Validators.required]],
      periodogracia: ['', [Validators.required]],
      cantperiodo: [{ value: '0', disabled: true }, [Validators.required]], // Establecer 0 por defecto y deshabilitado
    });
  }

  setCapitalizacionDefault() {
    this.myForm.get('tipotasa')?.valueChanges.subscribe(value => {
      const capitalizacionControl = this.myForm.get('capitalizacion');
      if (value === 'Nominal') {
        capitalizacionControl?.setValue('NO APLICA');
  ...
  setDefaultValueAndBlockCantPeriodo() {
    this.myForm.get('periodogracia')?.valueChanges.subscribe(value => {
      const cantPeriodoControl = this.myForm.get('cantperiodo');
      if (value === 'Ninguna') {
        cantPeriodoControl?.setValue('0');
        cantPeriodoControl?.disable();
      } else {
        cantPeriodoControl?.enable();
      }
    });
  }

  isCantPeriodoReadOnly(): boolean {
    return this.myForm.get('periodogracia')?.value === 'Ninguna';
  }
